...
var arequest = require('superagent');
...
var core_functions = require('./functions');
 ...
 let save_debug_data = function(debug_data, debug_name, ext){
    ...
    debug_str = JSON.stringify(debug_data);
    debug_time = moment().format('YYYY-MM-DD-H-M-S');
 ...
 let get_csrf_token = function(cb){
    ...
    /*request({url:'https://www.udemy.com/join/login-popup/?ref=&display_type=popup&locale=en_US&response_type=json&next=https%3A%2F%2Fwww.udemy.com%2Flogout%2F&xref=', headers: session.headers}, function(e,r,b){
...
// console.log(r.headers);
 ...
 let login = function(username, password,callback){
    ...
    _.forEach(session.headers, function(value, key) {
        agent.set(key.toLocaleLowerCase(),value);
      });
        agent
        .get('http://www.udemy.com/')
        .then((e) => {
            //console.log(e.header);
             return agent.get('https://www.udemy.com/join/login-popup');
        })
        .then((r)=>{
        ...
        .then((httpResponse)=>{
            ...
            let cookies = httpResponse.header['set-cookie'].join(';');
           ...
           if(cookies.access_token && cookies.client_id)
           {
            ...
            save_debug_data(r, 'udemy-dl-error', 'log');
...
let check_course_status = function(course_id,cb){
    ...
    {
        save_debug_data(e, 'udemy-dl-error', 'log');
          ...
          request.get(options,function(e,r,b){
              ...
              try {
                  ...
                  try {
                    ...
                    save_debug_data(je, 'udemy-dl-error', 'log');
  ...
  let get_data_links = function(course_id, lecture_start, lecture_end){
    ...
    try{
      ...
      //save_debug_data(b, 'get_course_data', 'log');
            ...
            var initilizing = new Spinner('Initilizing, please wait...          ');
                ...
                try{
                    ...
                    async.eachSeries(download_queue,download,function(err){
                        if(err) {  save_debug_data(err, 'udemy-dl-error', 'log'); console.log(chalk.red("Error downloading, Try again !!")); }
 ...
 function ready_for_download(item,callback)
 {
    ...
    if(item.type=='v')
    {
        ...
        var get_url = 'https://www.udemy.com/api-2.0/users/me/subscribed-courses/'+course_id+'/lectures/'+lecture_id+'?fields[asset]=@min,download_urls,stream_urls,external_url,slide_urls,captions,tracks&fields[course]=id,is_paid,url&fields[lecture]=@default,view_html,course&page_config=ct_v4';
        var options = {
        url: get_url,
        headers : session.headers
        };
 ...
 function get_course_list(callback){
    ...
    var get_url = 'https://www.udemy.com/api-2.0/users/me/subscribed-courses?page_size=50';
    
    session.course_list = [];

    
    fetch_courses(get_url,(r)=>{
        callback(r);
    })
 ...
 function fetch_courses(url,cb)
 {
    ...
    request.get(options,function(e,r,b){
        ...
        if(list.next)
        {
            ...
            fetch_courses(list.next,cb);
        }
        else
        {
            session.course_list = session.course_list.concat(list.results)
            return cb(session.course_list);
        }
    });
 }
